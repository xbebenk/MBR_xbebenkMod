...
Func getArmyCCSiegeMachines($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	Local $sCCSiegeDiamond = GetDiamondFromRect("628,470,700,555") ; Contains iXStart, $iYStart, $iXEnd, $iYEnd
	...
	Local $sSiegeInfo = getArmySiegeCap(640, 439, $bNeedCapture) ; OCR read Siege built and total
	...
	If UBound($aCurrentCCSiegeMachines, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentCCSiegeMachines, 1) - 1 ; Loop through found Troops
			...
			$g_aiCurrentCCSiegeMachines[$iCCSiegeIndex] = Number(getBarracksNewTroopQuantity(651, 469, $bNeedCapture)) ; Get The Quantity of the Troop, Slot() Does return the exact spot to read the Number from
			...
			If $g_bDebugSetlogTrain Then Setlog($sCCSiegeName & " Slot (" & 651 & "," & 469 & ")")
