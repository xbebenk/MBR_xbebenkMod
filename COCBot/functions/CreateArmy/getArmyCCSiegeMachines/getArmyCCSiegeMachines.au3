...
Func getArmyCCSiegeMachines($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	ElseIf $bOpenArmyWindow Then
		If Not OpenArmyOverview("getArmyCCSiegeMachines()") Then
			SetError(2)
	...
	Local $sCCSiegeDiamond = GetDiamondFromRect("600,470,660,530") ; Contains iXStart, $iYStart, $iXEnd, $iYEnd
	...
	Local $sSiegeInfo = getArmySiegeCap(615, 428, $bNeedCapture) ; OCR read Siege built and total
	...
	If UBound($aCurrentCCSiegeMachines, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentCCSiegeMachines, 1) - 1 ; Loop through found Troops
			...
			$g_aiCurrentCCSiegeMachines[$iCCSiegeIndex] = Number(getBarracksNewTroopQuantity(610, 456, $bNeedCapture)) ; Get The Quantity of the Troop, Slot() Does return the exact spot to read the Number from
			...
			If $g_bDebugSetlogTrain Then Setlog($sCCSiegeName & " Slot (" & 610 & "," & 456 & ")")
