						...
						For $i = 0 To UBound($aAvailableTroops_NXQ) - 1
							...
							If _CompareTexts($aAvailableTroops_NXQ[$i][0], $sTroopName, 80) Then ;We Just Need To redo the ocr for mentioned troop only
								...
								Else
									...
									If $aAvailableTroops_NXQ[$i][4] < 1 Then $aAvailableTroops_NXQ[$i][4] = Number(getTroopCountBig(Number($aAvailableTroops_NXQ[$i][1]), 640 - 7)) ; For Big numbers when the troop is selected
								EndIf
							EndIf
						Next
